...
//---------------------------------------------------------------------------------------------------------------
// Bellman–Held–Karp algorithm exact TSP or AcGePointXd
template <typename T>
static std::vector<size_t> PyGePointArrayShortestTourImpl(const T& pnts)
{
...
static PyGePoint2dArray PyGePoint2dArrayShortestTour(const PyGePoint2dArray& pnts)
{
    ...
    for (const auto& idx : PyGePointArrayShortestTourImpl(pnts))
...
static void makePyGeMatrix2dWrapper()
{
    ...
    class_<AcGeMatrix2d>("Matrix2d")
        ...
        .def("isSingular", &AcGeMatrix2d::isSingular, DS.ARGS({ "val: PyGe.Tol = ..." }), arg("AcGeTol") = getTol())
...
static PyGePoint3dArray PyGePoint3dConcaveHull(const PyGePoint3dArray& pnts, double concavity, double lengthThreshold)
{
    ...
    pylist.reserve(indexes.size());
    for (size_t idx : indexes)
        pylist.push_back(pnts[idx]);
    return pylist;
}

static boost::python::list PyGePoint3dConcaveHullIndexes(const PyGePoint3dArray& pnts, double concavity, double lengthThreshold)
{
    PyAutoLockGIL lock;
    boost::python::list pylist;
    for (size_t idx : Concave::concaveman<16>(pnts, concavity, lengthThreshold))
        pylist.append(idx);
    return pylist;
}
...
static void makePyGePoint3dWrapper()
{
    ...
    class_<PyGePoint3dArray>("Point3dArray")
        ...
        .def("sortByDynamicDistance", &sortByDynamicDistance, DSPA.ARGS({ "basePnt: PyGe.Point3d" }, 19145))
...
static void makePyGeMatrix3dWrapper()
{
    ...
    class_<AcGeMatrix3d>("Matrix3d")
        ...
        .def("isSingular", &AcGeMatrix3d::isSingular, DS.ARGS({ "val: PyGe.Tol = ..." }), arg("AcGeTol") = getTol())
