...
void makePyGiCommonDrawWrapper()
{
    ...
    class_<PyGiCommonDraw, bases<PyRxObject>>("CommonDraw", boost::python::no_init)
        .def("regenType", &PyGiCommonDraw::regenType, DS.ARGS(12946))
        .def("regenAbort", &PyGiCommonDraw::regenAbort, DS.ARGS(12945))
        .def("isDragging", &PyGiCommonDraw::isDragging, DS.ARGS(12942))
        .def("subEntityTraits", &PyGiCommonDraw::subEntityTraits, DS.ARGS(12948))
...
void makePyGiWorldDrawWrapper()
{
    ...
    class_<PyGiWorldDraw, bases<PyGiCommonDraw>>("WorldDraw", boost::python::no_init)
        .def("geometry", &PyGiWorldDraw::geometry, DS.ARGS(14119))
...
void makePyGiViewportDrawWrapper()
{
    ...
    class_<PyGiViewportDraw, bases<PyGiCommonDraw>>("ViewportDraw", boost::python::no_init)
        .def("geometry", &PyGiViewportDraw::geometry, DS.ARGS(13999))
...
void makePyGiGeometryWrapper()
{
    ...
    class_<PyGiGeometry, bases<PyRxObject>>("Geometry", boost::python::no_init)
        ...
        .def("getWorldToModelTransform", &PyGiGeometry::getWorldToModelTransform, DS.ARGS(13160))
        .def("pushModelTransform", &PyGiGeometry::pushModelTransform1)
        .def("pushModelTransform", &PyGiGeometry::pushModelTransform2, DS.ARGS({ "val : PyGe.Vector3d|PyGe.Matrix3d" }, 13173))
        .def("popModelTransform", &PyGiGeometry::popModelTransform, DS.ARGS(13171))
        .def("pushPositionTransform", &PyGiGeometry::pushPositionTransform1)
        .def("pushPositionTransform", &PyGiGeometry::pushPositionTransform2, DS.ARGS({ "behavior : PyGi.AcGiPositionTransformBehavior"," offset : PyGe.Point2d|PyGe.Point2d" }, 13175))
        ...
        .def("ray", &PyGiGeometry::ray, DS.ARGS({ "p1 : PyGe.Point3d",  "p2 : PyGe.Point3d" }, 13177))
        .def("pline", &PyGiGeometry::pline1)
        .def("pline", &PyGiGeometry::pline2, DS.ARGS({ "p1 : PyDb.Polyline",  "fromIndex : int=0","numSegs : int=0" }, 13164))
        .def("rowOfDots", &PyGiGeometry::rowOfDots, DS.ARGS({ "count : int","start: PyGe.Point3d" ,"step : PyGe.Vector3d" }, 13178))
        .def("ellipticalArc", &PyGiGeometry::ellipticalArc1)
        .def("ellipticalArc", &PyGiGeometry::ellipticalArc2, DS.ARGS({ "center:PyGe.Point3d","norm:PyGe.Vector3d",
            "majAxisLen:float","minAxisLen:float","startDeg:float","endDeg:float","tilt:float", "arcType:ArcType=kAcGiArcSimple" }, 13158))
        .def("worldLine", &PyGiGeometry::worldLine, DS.ARGS({ "p1 : PyGe.Point3d",  "p2 : PyGe.Point3d" }, 13182))
        .def("edge", &PyGiGeometry::edge, DS.ARGS({ "edgeList : list[PyGe.Curve2d]" }, 13157))
        .def("draw", &PyGiGeometry::draw, DS.ARGS({ "drawable : PyGi.Drawable" }, 13156))
...
{
    using Iter = boost::python::stl_input_iterator<PyGeCurve2d>;
    AcArray<AcGeCurve2d*> _edges;
    for (Iter it(edges), end; it != end; ++it) {
        _edges.append(it->impObj());
    }
