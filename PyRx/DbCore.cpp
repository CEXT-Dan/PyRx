...
void makeDbCoreWrapper()
{
    PyDocString DS("Core");
    class_<DbCore>("Core")
        .def("attachPointCloudExEntity", &DbCore::attachPointCloudExEntity, DS.ARGS({ "path: str","pos: PyGe.Point3d","scale: float","rotation: float","db: PyDb.Database" })).staticmethod("attachPointCloudExEntity")
        ...
        .def("findField", &DbCore::findField, DS.SARGS({ "val: str","from: int" })).staticmethod("findField")
        ...
        .def("getUnitsConversion", &DbCore::getUnitsConversion, DS.SARGS({ "from: PyDb.UnitsValue","to: PyDb.UnitsValue" })).staticmethod("getUnitsConversion")
