...
void makePyGeEntity3dWrapper()
{
    ...
    class_<PyGeEntity3d>("Entity3d", boost::python::no_init)
        ...
        .def("rotateBy", &PyGeEntity3d::rotateBy2, DS.ARGS({ "angle : double","vec : PyGe.Vector3d","pt : PyGe.Point3d = origin" }))
        .def("mirror", &PyGeEntity3d::mirror, DS.ARGS({ "val : PyGe.Plane" }))
        .def("scaleBy", &PyGeEntity3d::scaleBy1)
        .def("scaleBy", &PyGeEntity3d::scaleBy2, DS.ARGS({ "scaleFactor : double","pt : PyGe.Point3d = origin" }))
