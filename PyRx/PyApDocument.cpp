...
void makePyApDocumentWrapper()
{
    ...
    class_<PyApDocument, bases<PyRxObject>>("Document", boost::python::no_init)
        .def("database", &PyApDocument::database, DS.CLASSARGS())
        .def("editor", &PyApDocument::editor, DS.CLASSARGS())
        .def("fileName", &PyApDocument::fileName, DS.CLASSARGS())
        .def("formatForSave", &PyApDocument::formatForSave, DS.CLASSARGS())
        .def("lockMode", &PyApDocument::lockMode1)
        .def("lockMode", &PyApDocument::lockMode2, DS.CLASSARGS({"bIncludeMyLocks : bool=False"}))
        .def("myLockMode", &PyApDocument::myLockMode,DS.CLASSARGS())
        .def("isQuiescent", &PyApDocument::isQuiescent, DS.CLASSARGS())
        .def("docTitle", &PyApDocument::docTitle, DS.CLASSARGS())
        .def("setDocTitle", &PyApDocument::setDocTitle, DS.CLASSARGS({"title : str"}))
        .def("getCountOfLispList", &PyApDocument::getCountOfLispList, DS.CLASSARGS())
        .def("getItemOfLispList", &PyApDocument::getItemOfLispList, DS.CLASSARGS({"index : int"}))
        .def("isCommandInputInactive", &PyApDocument::isCommandInputInactive, DS.CLASSARGS())
        .def("pushDbmod", &PyApDocument::pushDbmod, DS.CLASSARGS())
        .def("popDbmod", &PyApDocument::popDbmod, DS.CLASSARGS())
        .def("upgradeDocOpen", &PyApDocument::upgradeDocOpen, DS.CLASSARGS())
        .def("downgradeDocOpen", &PyApDocument::downgradeDocOpen, DS.CLASSARGS({"promptForSave : bool"}))
        .def("transactionManager", &PyApDocument::transactionManager,  DS.CLASSARGS())
        .def("inputPointManager", &PyApDocument::inputPointManager, DS.CLASSARGS())
        //static
        .def("docWnd", &PyApDocument::docWnd, DS.CLASSARGSSTATIC()).staticmethod("docWnd")
        .def("className", &PyApDocument::className, DS.CLASSARGSSTATIC()).staticmethod("className")
        ;
