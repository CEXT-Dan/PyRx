...
void makePyApDocManagerWrapper()
{
    ...
    class_<PyApDocManager, bases<PyRxObject>>("DocManager")
        ...
        .def("sendStringToExecute", &PyApDocManager::sendStringToExecute2, DS.ARGS({ "doc: PyAp.Document","script: str","bActivate: bool = True","bWrapUpInactiveDoc: bool = False","bEchoString: bool = True" }))
        .def("appContextNewDocument", &PyApDocManager::appContextNewDocument, DS.ARGS({ "templateName : str" }))
        .def("appContextOpenDocument", &PyApDocManager::appContextOpenDocument, DS.ARGS({ "dwgName : str" }))
        .def("appContextRecoverDocument", &PyApDocManager::appContextRecoverDocument, DS.ARGS({ "dwgName : str" }))
        .def("appContextPromptOpenDocument", &PyApDocManager::appContextPromptOpenDocument, DS.ARGS())
        .def("appContextCloseDocument", &PyApDocManager::appContextCloseDocument, DS.ARGS({ "doc: PyAp.Document" }))
        .def("newDocument", &PyApDocManager::newDocument, DS.ARGS())
        .def("openDocument", &PyApDocManager::openDocument, DS.ARGS())
        .def("closeDocument", &PyApDocManager::closeDocument, DS.ARGS({ "doc: PyAp.Document" }))
        .def("inputPending", &PyApDocManager::inputPending, DS.ARGS({ "doc: PyAp.Document" }))
        .def("disableDocumentActivation", &PyApDocManager::disableDocumentActivation, DS.ARGS())
        .def("enableDocumentActivation", &PyApDocManager::enableDocumentActivation, DS.ARGS())
        .def("isDocumentActivationEnabled", &PyApDocManager::isDocumentActivationEnabled, DS.ARGS())
        .def("documentCount", &PyApDocManager::documentCount, DS.ARGS())
