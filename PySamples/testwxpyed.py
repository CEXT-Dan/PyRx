...
class TestDialog(wx.Dialog):
    def __init__(
        ...
        self.sel = wx.Button(self,-1, "Select")
        self.sel.SetDefault()
        self.Bind(wx.EVT_BUTTON, self.onButton, self.sel)
        sizer.Add(self.sel)
    ...
    def onButton(self, event):
        val = PyAp.Application().docManager().curDocument().editor().entsel("\nEntSel\n")
        if val[0] == PyEd.PromptStatus.eNormal :
            print(val[1].objectClass().name())
